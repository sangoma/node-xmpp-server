  ...
  if (!this.authenticated) {
    if (this.server && this.server.availableSaslMechanisms) {
      ...
      if (opts && opts.tls && !this.connection.isSecure) {
        features
          .c('starttls', this.connection.NS_XMPP_TLS)
          .c('required')
      }
    ...
    var mechanismsEl = features.c('mechanisms', NS_XMPP_SASL)
    this.mechanisms.forEach(function (mech) {
      mechanismsEl.c('mechanism').t(mech.prototype.name)
    })
  } else {
    features.c('bind', NS_BIND)
    // see C2S server sendSessionFeature option
    if (this.server.sendSessionFeature === true) {
      features.c('session', NS_SESSION)
    }
  }
  this.send(features)
}
...
Session.prototype.onStanza = function (stanza) {
  ...
  if (stanza.is('starttls', this.connection.NS_XMPP_TLS)) {
    var toSend = new Element(
      'proceed', this.connection.NS_XMPP_TLS
    )
    this.send(toSend)
...
Session.prototype.sendAuthError = function (error) {
  ...
  this.send(new Element('failure', NS_XMPP_SASL)
    .c(type).up()
    .c('text').t(message)
  )
}
...
Session.prototype.onAuth = function (stanza) {
  ...
  if (!this.mechanism) {
    ...
    this.mechanism.success = function (user) {
      ...
      self.send(new Element('success', NS_XMPP_SASL))
    ...
    this.emit('register', {
      ...
      if (!error) {
        ...
        reply
          ...
          .c(error.condition, NS_STANZAS).up()
          .c('text', NS_STANZAS)
          .t(error.message)
      }
      proceed()
