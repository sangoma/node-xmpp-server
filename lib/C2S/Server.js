 ...
 *   options.sendSessionFeature: send obsolete session stream:feature (defaults true)
...
function C2SServer (options) {
  ...
  // was mandatory for C2S in RFC 3920 but then removed in RFC 6120
  // for being considered useless altough some clients might still require it
  this.sendSessionFeature = !(options && options.sendSessionFeature === false)
